...
class Utils:
    ...
    def parse_vector(line, name):
        split = line.replace(f'{name}=', '').replace('(', '').replace(')', '').split(',')
...
class MapImporter:
    ...
    def import_terrains(self, terrains):
        for ter in terrains:
            ...
            img = bpy.data.images.load(os.path.join(self.level_dir, 'Terrains', ter.map, ter.name, 'HeightMap.png'))
...
importer = MapImporter(source_dir='E:\\TERA_DEV\\test_re_export\\cli', map_name='RNW_C_P')
